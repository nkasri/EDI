...
"""
AUTHORS: KARIM NASRI AND SEIF EDDINE DJELLOUL
"""
...
def error_response_custom(code, msg, name, exp_type):
    return {
        "jsonrpc": "2.0",
        "id": None,
        "error": {
            "code": code,
            "message": msg,
            "exception_type": exp_type
            }
        }

class HCMAPI(http.Controller):
    @http.route(
        '/auth/',
        type='json', auth='none', methods=["POST", "OPTIONS"], cors='*', csrf=False)
    ...
    @http.route(
        ...
        type='json', auth='user', methods=["POST"], cors='*', csrf=False)
    ...
    @http.route(
        '/api/<string:model>',
        type='http', auth='user', methods=['GET'], cors='*', csrf=False)
    def get_model_data(self, model, **params):
        ...
        try:
            ...
            res = error_response_custom(400, e.msg, "Field don't existe","Field don't existe")
            return http.Response(
                json.dumps(res),
                status=200,
                mimetype='application/json'
    ...
    @http.route(
        '/api/<string:model>/<int:rec_id>/',
        type='json', auth="user", methods=['PUT'], cors='*', csrf=False)
    ...
    @http.route(
        '/api/<string:model>/<int:rec_id>/',
        type='http', auth="user", methods=['DELETE'], cors='*', csrf=False)
